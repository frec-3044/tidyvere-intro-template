...
alternatively your can download the file and read it in from your computer.  If you run this code, you should only download it once and then put a comment # in first of the download line so that it does not repeatedly download the code.

```{r eval = FALSE}
download.file(url = file_name, destfile = "assignment2_water_quality_raw.csv", method = "curl")
```

You will then need to read the file from your local directory. 

```{r eval = FALSE}
df <- read_csv("assignment2_water_quality_raw.csv")
```
...
Remember to Knit your document as a `github_document` and comment+push your code, knitted document, png (from question 15), and csv (from question 16) files to GitHub.  
